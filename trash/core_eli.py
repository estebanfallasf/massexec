...
def command_introducer(commands_array,s,ssh,d):
	...
	if commands_i.find('7PASSWD7') > 0:
		...
		print ("Introducing last successful password...("+real_password[0:3]+"...)")
...
#Try to write in the file introduced by user.
#If not error (exception) it continues.
#Else it exits.
try:
	report_f = open(out, 'a',0)
except IOError:
	print ("File "+out+" does not exist or you do not have permissions.\n")
